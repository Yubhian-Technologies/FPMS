...
const Appeals = () => {
  ...
  const groupedAppeals = appeals.reduce((acc: Record<string, Appeal[]>, appeal) => {
            ...
            <CardContent className="grid gap-3">
              ...
              <div className="grid gap-1">
                ...
                <Select value={formData.subId} onValueChange={v => handleInputChange("subId", v)} disabled={!subsections.length}>
                  ...
                  <SelectContent>
                    {subsections.map(sub => (
                      <SelectItem key={sub.id} value={sub.id}>{sub.id}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              ...
              {formData.criterionName && (
                <div className="bg-gray-50 p-2 rounded-md text-xs space-y-1">
                  <div><strong>Faculty Score:</strong> {criteriaOptions.find(c => c.name === formData.criterionName)?.claimedScore}</div>
                  <div><strong>Faculty Description:</strong> {criteriaOptions.find(c => c.name === formData.criterionName)?.facultyDescription}</div>
                  <div><strong>HOD Score:</strong> {criteriaOptions.find(c => c.name === formData.criterionName)?.hodScore ?? "N/A"}</div>
                  <div><strong>HOD Description:</strong> {criteriaOptions.find(c => c.name === formData.criterionName)?.hodDescription ?? "N/A"}</div>
                </div>
              )}
              ...
              <div className="grid gap-1">
                ...
                <Textarea value={formData.evidence} onChange={e => handleInputChange("evidence", e.target.value)} placeholder="Paste link or text evidence" className="text-sm h-16" />
              </div>

              <div className="grid gap-1">
                <Label className="text-xs">Appeal Reason</Label>
                <Textarea value={formData.appealReason} onChange={e => handleInputChange("appealReason", e.target.value)} placeholder="Explain why you are appealing this score" className="text-sm h-16" />
              </div>
